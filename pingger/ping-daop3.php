...
$d_koridor=$conn->query("SELECT (id_koridor) FROM koridor WHERE pusat_client='1'")->fetch_assoc();
$id_st = $d_koridor['id_koridor'];
$limit = 5; // jumlah Ip per Halaman
$start = 1;
$slice = 9;
$self_server = "./home&id=$id_st";
$q = "SELECT * FROM client WHERE id_koridor='$id_st'";
$r = $conn->query($q);
...
$q = "SELECT * FROM client WHERE id_koridor='$id_st' LIMIT $limitvalue, $limit";
...
if ($cek_page->num_rows != 0) {
    ...
    if ($r->num_rows!==0) {
            ...
            while ($client=$r->fetch_assoc()) {
                ...
                echo "<tr>
                <td>$no</td>
                <td><strong>$nama_halte</strong></td>
                <td>$ip_client</td>
                <td>$status</td>
                <td>
    ...
    $sql_cek_row = "SELECT*FROM client WHERE id_koridor='$id_st'";
